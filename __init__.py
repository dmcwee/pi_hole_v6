...
async def async_setup_entry(hass: HomeAssistant, entry: PiHoleConfigEntry) -> bool:
    ...
    # V6 attributes
    name_to_key = {
        "Core Update Available": "core_update",
        "Web Update Available": "web_update",
        "FTL Update Available": "ftl_update",
        "Status": "status",
        "Ads Blocked Today": "ads_blocked_today",
        "Ads Percentage Blocked Today": "ads_percentage_today",
        "Seen Clients": "clients_ever_seen",
    ...
    async def async_update_data() -> None:
        ...
        try:
            _LOGGER.debug("Updating data from Pi-hole API")
            await api.get_data()
            await api.get_versions()
        except HoleError as err:
            _LOGGER.error("Failed to communicate with API: %s", err)
            raise UpdateFailed(f"Failed to communicate with API: {err}") from err
        if not isinstance(api.data, dict):
            _LOGGER.debug("API data is not a dictionary")
